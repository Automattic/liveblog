...
describe( 'user reducer', () => {
	...
	it( 'should handle ENTRY_EDIT_OPEN', () => {
		expect( user( initialState, entryEditOpen( id ) ) ).toEqual(
	...
	const stateAfterEntryEditClose = {
		...stateAfterEntryEditOpen,
		entries: {
			...stateAfterEntryEditOpen.entries,
			[ id ]: { isEditing: false },
		},
	};

	it( 'should handle ENTRY_EDIT_CLOSE', () => {
		expect( user( stateAfterEntryEditOpen, entryEditClose( id ) ) ).toEqual(
			stateAfterEntryEditClose
		);
	} );
} );
