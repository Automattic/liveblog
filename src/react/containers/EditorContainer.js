...
class EditorContainer extends Component {
					...
					next: ( res ) => {
						...
						if ( isDefaultLoad && options.length > 10 ) {
							...
							options.push( {
								...
								key: '__hint__',
								name: __(
									'Type to search for more authors…',
									'liveblog'
								),
								isDisabled: true,
								isHint: true,
	...
	handleOnSearch( trigger, text ) {
		...
		switch ( trigger ) {
			...
			case '#':
				...
				break;
			case '/':
				this.filterCommandSuggestions(
					config.autocomplete[ 0 ].data,
					text
				);
				break;
			case ':':
				this.filterEmojiSuggestions(
					config.autocomplete[ 1 ].data,
					text
				);
				break;
			default:
	...
	render() {
		...
		return (
			...
			>
				{ ! isEditing && (
					<h1 className="liveblog-editor-title">
				...
				{ mode === 'editor' && (
					<React.Suspense
						fallback={
							<div className="liveblog-editor-loading">
								{ __( 'Loading editor…', 'liveblog' ) }
							</div>
						}
					>
						<LexicalEditor
							...
							onSearch={ ( trigger, text ) =>
								this.handleOnSearch( trigger, text )
							}
				...
				<h2 className="liveblog-editor-subTitle">
					{ __( 'Authors:', 'liveblog' ) }
				</h2>
				<Async
					...
					noOptionsMessage={ ( { inputValue } ) =>
						inputValue
							? __( 'No authors matched', 'liveblog' )
				...
				>
					{ isEditing
						? __( 'Publish Update', 'liveblog' )
						: __( 'Publish New Entry', 'liveblog' ) }
				</button>
			</div>
		);
	}
