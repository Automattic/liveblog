...
class EditorContainer extends Component {
	...
	handleImageUpload(file) {
		...
		return new Promise((resolve) => {
			uploadImage(formData, config).pipe(
				timeout(60000),
				map((res) => res.response),
			).subscribe({
                next: res => {
	...
	render() {
		const { editorState, suggestions, mode, authors, readOnly } = this.state;
		...
		return (
			<div className="liveblog-editor-container">
				...
				<Async
					isMulti
					value={authors}
					onInputChange={this.onSelectAuthorChange.bind(this)}
