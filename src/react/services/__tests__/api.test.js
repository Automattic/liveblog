...
describe( 'api service', () => {
	describe( 'getEntries', () => {
		...
		it( 'should use "latest" for page 1 to avoid stale cached data', () => {
			...
			expect( result.url ).toBe(
				'https://example.com/wp-json/liveblog/v1/123/get-entries/1/latest'
			);
		} );
		...
		it( 'should use newestEntry for page 2+ for consistent pagination', () => {
			...
			expect( result.url ).toBe(
				'https://example.com/wp-json/liveblog/v1/123/get-entries/2/150-1734567900'
			);
		} );
		...
		it( 'should fall back to config values for page 2+ when newestEntry is empty', () => {
			...
			expect( result.url ).toBe(
				'https://example.com/wp-json/liveblog/v1/123/get-entries/3/100-1734567800'
			);
		} );
	} );
	...
	describe( 'polling', () => {
		...
		it( 'should round both start and end timestamps to bucket boundaries', () => {
			...
			expect( result.url ).toBe(
				'https://example.com/wp-json/liveblog/v1/123/entries/1734567880/1734567890/'
			);
		} );
		...
		it( 'should handle zero newestEntryTimestamp', () => {
			...
			expect( result.url ).toBe(
				'https://example.com/wp-json/liveblog/v1/123/entries/0/1734567890/'
			);
		} );
		...
		it( 'should handle different refresh intervals', () => {
			...
			expect( result.url ).toBe(
				'https://example.com/wp-json/liveblog/v1/123/entries/1734567870/1734567870/'
			);
		} );
	} );
} );
