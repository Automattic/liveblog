...
class WPCOM_Liveblog_Socketio_Loader {
	...
	public static function show_old_php_for_socketio_error() {
		...
		$format_string = __( 'Your current PHP is version %1$s, which is too old to run the Liveblog plugin with WebSocket support enabled. The minimum required version is %2$s. Please, either update PHP or disable WebSocket support by removing or setting to false the constant LIVEBLOG_USE_SOCKETIO in wp-config.php.', 'liveblog' );
		$message       = sprintf( $format_string, PHP_VERSION, self::SOCKETIO_MIN_PHP_VERSION );
