...
final class WPCOM_Liveblog {
	...
	public static function load() {
		load_plugin_textdomain( 'liveblog', false, dirname( plugin_basename( LIVEBLOG_PLUGIN_FILE ) ) . '/languages/' );
	...
	private static function includes() {
		require __DIR__ . '/class-wpcom-liveblog-entry.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-query.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-key-events.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-key-events-widget.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-extend.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-extend-feature.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-extend-feature-hashtags.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-extend-feature-commands.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-extend-feature-emojis.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-extend-feature-authors.php';
		require __DIR__ . '/class-wpcom-liveblog-lazyloader.php';
		require __DIR__ . '/class-wpcom-liveblog-socketio-loader.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-embed.php';
		require __DIR__ . '/class-wpcom-liveblog-entry-embed-sdks.php';
		require __DIR__ . '/class-wpcom-liveblog-amp.php';
		require __DIR__ . '/class-wpcom-liveblog-amp-template.php';

		if ( self::use_rest_api() ) {
			require __DIR__ . '/class-wpcom-liveblog-rest-api.php';
		}
		...
		if ( defined( 'WP_CLI' ) && WP_CLI ) {
			require __DIR__ . '/class-wpcom-liveblog-wp-cli.php';
		}

		require __DIR__ . '/class-wpcom-liveblog-cron.php';
	}
	...
	public static function init() {
		 ...
		 *
		 * By default, only the 'post' post type supports liveblog. Use this filter
		 * to add support for pages, custom post types, or other post types.
		 *
		 * @since 1.10.0
		 *
		 * @param string[] $post_types Array of post type names. Default array( 'post' ).
		 */
	 ...
	 *
	 * @param int|null $post_id The post ID to check.
	 * @return string|false One of: 'enable', 'archive', false.
	 */
	...
	public static function ajax_crud_entry() {
		...
		$crud_action = isset( $_POST['crud_action'] ) ? sanitize_text_field( wp_unslash( $_POST['crud_action'] ) ) : 0;
		// phpcs:enable
		...
		$args['contributor_ids'] = isset( $_POST['contributor_ids'] ) ? sanitize_text_field( wp_unslash( $_POST['contributor_ids'] ) ) : false;
		// phpcs:enable
	...
	public static function get_single_entry( $entry_id ) {
		...
		if ( ! empty( $entries_for_json ) ) {
			// Entries found.
			$result_for_json['index']             = (int) filter_input( INPUT_GET, 'index', FILTER_SANITIZE_NUMBER_INT );
	 ...
	 * @return array|false An array of json encoded results or false.
	 */
	...
	/**
	 * Handle AJAX preview entry request.
	 *
	 * @return void
	 */
	 ...
	 */
	public static function add_comment_class( $classes, $css_class, $comment_id ) {
	...
	public static function admin_enqueue_scripts( $hook_suffix ) {
		...
		if ( in_array( $hook_suffix, array( 'post.php', 'post-new.php' ), true ) && post_type_supports( get_post_type(), self::KEY ) ) {
			...
			// Load dependencies from .asset.php file generated by @wordpress/scripts.
			$asset_file   = include plugin_dir_path( LIVEBLOG_PLUGIN_FILE ) . 'build/admin.asset.php';
			...
			wp_enqueue_style( self::KEY, plugins_url( 'build/admin.css', LIVEBLOG_PLUGIN_FILE ), array(), $version );
			wp_enqueue_script( 'liveblog-admin', plugins_url( 'build/admin.js', LIVEBLOG_PLUGIN_FILE ), $dependencies, $version, false );
			wp_localize_script(
				'liveblog-admin',
			...
			// Also set liveblog_settings for admin context (needed by webpack_public_path).
			wp_localize_script(
				'liveblog-admin',
				'liveblog_settings',
				array(
					'plugin_dir' => plugin_dir_url( LIVEBLOG_PLUGIN_FILE ),
				)
			);
		}
	}
	...
	public static function enqueue_scripts() {
		...
		// Load dependencies from .asset.php file generated by @wordpress/scripts.
		$asset_file   = include plugin_dir_path( LIVEBLOG_PLUGIN_FILE ) . 'build/app.asset.php';
		$dependencies = isset( $asset_file['dependencies'] ) ? $asset_file['dependencies'] : array();
		$version      = isset( $asset_file['version'] ) ? $asset_file['version'] : self::VERSION;
		...
		if ( apply_filters( 'liveblog_load_default_styles', true ) ) {
			wp_enqueue_style( self::KEY, plugins_url( 'build/app.css', LIVEBLOG_PLUGIN_FILE ), array(), $version );
		}

		// Load client scripts.
		wp_enqueue_script( self::KEY, plugins_url( 'build/app.js', LIVEBLOG_PLUGIN_FILE ), $dependencies, $version, true );
	...
	private static function add_default_plupload_settings() {
		...
		$settings = array(
			'defaults' => $defaults,
			'browser'  => array(
				'mobile'    => ( function_exists( 'jetpack_is_mobile' ) ? jetpack_is_mobile() : wp_is_mobile() ), // phpcs:ignore WordPressVIPMinimum.Functions.RestrictedFunctions.wp_is_mobile_wp_is_mobile -- Fallback when Jetpack is not available.
	...
	public static function add_liveblog_to_content( $content ) {
		...
		// We don't want to add the liveblog to other loops on the same page.
		if ( ! self::is_viewing_liveblog_post() ) {
			return $content;
		}
	...
	public static function get_template_part( $template_name, $template_variables = array() ) {
		...
		if ( file_exists( $child_theme_template ) ) {
			...
			include dirname( LIVEBLOG_PLUGIN_FILE ) . '/templates/' . $template_name;
		}
		return ob_get_clean();
	}
	...
	/**
	 * Handle image embed for liveblog entries.
	 *
	 * @param array  $matches  Regex matches.
	 * @param array  $attr     Shortcode attributes.
	 * @param string $url      The URL.
	 * @param array  $rawattr  Raw shortcode attributes.
	...
	/**
	 * Handle AJAX request to set liveblog state for a post.
	 *
	 * @return void
	 */
	 ...
	 * @param array  $request_vars Request variables.
	 *
	 * @return string|false The metabox markup or false on failure.
	 */
	 ...
	 *
	 * @return bool|void False on invalid state, void otherwise.
	 */
	public static function set_liveblog_state( $post_id, $new_state ) {
		...
		if ( null !== self::$auto_archive_days ) {
			...
			$current_state = get_post_meta( $post_id, self::KEY, false );

			// Instantiate an entry query object.
			$query            = new WPCOM_Liveblog_Entry_Query( $post_id, self::KEY );
			$latest_timestamp = ( null !== $query->get_latest_timestamp() ) ? $query->get_latest_timestamp() : strtotime( gmdate( 'Y-m-d H:i:s' ) );
			...
			if ( 'enable' === $new_state && ( 0 === count( $current_state ) || 'archive' === $current_state[0] ) ) {
		...
		if ( in_array( $new_state, array( 'enable', 'archive' ), true ) ) {
			...
			// Let's remove the autoarchive meta data too.
	 ...
	 * @return array Post filtered arguments.
	 */
	 ...
	 *
	 * @param string $action The nonce action to check.
	 * @return void
	 */
	...
	/**
	 * Send a forbidden error message.
	 *
	 * @param string $message The error message.
	 * @return void
	 */
	...
	private static function status_header_with_message( $status, $message ) {
		...
		$wp_header_to_desc[ $status ] = self::sanitize_http_header( $message ); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Temporarily modifying for custom status message.

		status_header( $status );

		$wp_header_to_desc[ $status ] = $official_message; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Restoring original value.
	}
	 ...
	 *
	 * @param string $text The header text to sanitize.
	 * @return string Sanitized header text.
	 */
	 ...
	 */
	public static function protect_liveblog_meta_key( $is_protected, $meta_key ) {
	 ...
	 *
	 * @param string $size Human-readable size (e.g., '1K', '2M', '3G').
	...
	/**
	 * Check if the current WordPress version is too old for liveblog.
	 *
	 * @return bool Whether the WordPress version is too old.
	 */
	...
	/**
	 * Print liveblog metadata as JSON-LD script.
	 *
	 * @return void
	 */
