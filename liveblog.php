...
if ( ! class_exists( 'WPCOM_Liveblog' ) ) :
	...
	final class WPCOM_Liveblog {
		...
		public static function load() {
			...
			// Initialize DDD services via the bootstrapper (centralises hook wiring).
			$container    = \Automattic\Liveblog\Infrastructure\DI\Container::instance();
			$bootstrapper = new \Automattic\Liveblog\Infrastructure\WordPress\PluginBootstrapper( $container );
			$bootstrapper->init();
		}
		...
		private static function includes() {
			...
			require __DIR__ . '/classes/class-wpcom-liveblog-amp.php';
			require __DIR__ . '/classes/class-wpcom-liveblog-amp-template.php';

			if ( self::use_rest_api() ) {
				require __DIR__ . '/classes/class-wpcom-liveblog-rest-api.php';
			}
		...
		public static function admin_enqueue_scripts( $hook_suffix ) {
			...
			if ( in_array( $hook_suffix, array( 'post.php', 'post-new.php' ), true ) && post_type_supports( get_post_type(), self::KEY ) ) {
				...
				wp_enqueue_style( self::KEY, plugins_url( 'build/admin.css', __FILE__ ), array(), $version );
				wp_enqueue_script( 'liveblog-admin', plugins_url( 'build/admin.js', __FILE__ ), $dependencies, $version, false );
				wp_localize_script(
					'liveblog-admin',
		...
		public static function get_liveblog_metadata( $metadata, $post ) {
			...
			foreach ( $entries['entries'] as $entry ) {
				...
				// Add authors if available (entries can be authorless).
